...
<div class="min-h-screen lg:grid lg:grid-cols-[17rem_1fr]">
    ...
    <aside data-mobile-menu id="primary-navigation" class="fixed inset-y-0 left-0 z-40 hidden w-72 overflow-y-auto bg-slate-950 px-4 py-5 text-slate-200 lg:sticky lg:top-0 lg:block lg:h-screen lg:w-auto" aria-label="Hoofdnavigatie">
        ...
        <nav class="mt-8 space-y-6">
            ...
            @endif
            @if($canViewBanking || $canUploadBankImports)<x-navigation.section title="Bank">@if($canViewBanking)<x-navigation.item label="Banktransacties" :href="route('banking.payments.index')" :active="request()->routeIs('banking.payments.*')" /><x-navigation.item label="Bankafschriften" :href="route('banking.import.batches.index')" :active="request()->routeIs('banking.import.batches.*') || request()->routeIs('banking.import.statements.*')" /><x-navigation.item label="Te verwerken" :href="route('banking.reconciliation.index')" :active="request()->routeIs('banking.reconciliation.*')" />@endif @if($canUploadBankImports)<x-navigation.item label="Importeren" :href="route('banking.import.create')" :active="request()->routeIs('banking.import.create') || request()->routeIs('banking.import.preview')" />@endif</x-navigation.section>@endif
