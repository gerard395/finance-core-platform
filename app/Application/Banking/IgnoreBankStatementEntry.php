...
final readonly class IgnoreBankStatementEntry
{
        ...
        try {
            return $this->transactions->run(function () use ($administrationId, $entryId, $reason, $actorId): ManualReconciliationResult {
                ...
                if ($latest?->action === BankEntryManualAction::Ignore) {
                    return new ManualReconciliationResult(ManualReconciliationStatus::AlreadyIgnored);
                }
                $id = $this->ids->historyId();
                $event = new BankEntryReconciliationHistory($id, $administrationId, $entryId, BankEntryManualAction::Ignore, $latest?->id, $reason, $actorId, $this->clock->now());
