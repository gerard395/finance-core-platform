...
final readonly class AdministrationAuthorizationProvisioner
{
    private const string ASSIGNMENT_ID = 'c0ac832e-bb21-4dbd-a44b-504aff94000f';
    ...
    public function provision(): void
    {
        $this->transactions->run(function (): void {
            $permissionDefinition = AdministrationPermission::UpdateSettings;
            $roleDefinition = AdministrationRole::Manager;
            ...
            $permission = $this->permissions->findById($permissionDefinition->id()) ?? new Permission(
                $permissionDefinition->id(), $permissionDefinition->code(), $permissionDefinition->name(), null, PermissionStatus::Active,
            );
            if (! $permission->code()->equals($permissionDefinition->code())) {
                throw new LogicException('Administration permission identity conflicts with its canonical code.');
            }
            ...
            $role = $this->roles->findById($roleDefinition->id()) ?? new Role(
                $roleDefinition->id(), $roleDefinition->code(), $roleDefinition->name(), null, RoleStatus::Active,
            );
            if (! $role->code()->equals($roleDefinition->code())) {
                throw new LogicException('Administration role identity conflicts with its canonical code.');
            }
            ...
            $existing = RolePermissionRecord::query()
                ...
                ->where('permission_id', $permissionDefinition->id()->toString())
                ->first();
            if ($existing !== null) {
                $assignment = $this->assignments->findById(new RolePermissionId(new Uuid($existing->getAttribute('id'))));
                if ($assignment === null) {
                    throw new LogicException('Existing Administration role-permission assignment could not be loaded.');
                }
                $assignment->activate();
            ...
            if (RolePermissionRecord::query()->find($assignmentId->toString()) !== null) {
                throw new LogicException('Stable Administration role-permission identity belongs to another assignment.');
            }
    ...
    private function assertSameDefinition(?string $idByIdentity, ?string $idByCode, string $type): void
    {
        ...
        if ($idByIdentity !== null || $idByCode !== null) {
            throw new LogicException("{$type} definition is ambiguous for the canonical Administration definition.");
        }
    }
}
