...
final class EloquentBankTransactionRepository implements BankTransactionRepository
{
    ...
    {
        DB::table('bank_transactions')->updateOrInsert(['id' => $tx->id()->toString()], ['administration_id' => $tx->administrationId()->toString(), 'administration_bank_account_id' => $tx->bankAccountId()->toString(), 'transaction_date' => $tx->transactionDate()->value()->format('Y-m-d'), 'amount' => $tx->amount()->amount(), 'currency' => $tx->amount()->currency()->code(), 'reference' => $tx->reference()->value(), 'description' => $tx->description()->value(), 'status' => $tx->status()->value, 'created_by' => $tx->createdBy()->toString(), 'created_at' => $tx->createdAt(), 'finalized_by' => $tx->finalizedBy()?->toString(), 'finalized_at' => $tx->finalizedAt()]);
        $p = $tx->payment();
    ...
    {
        $p = DB::table('payments')->where('administration_id', $r->administration_id)->where('bank_transaction_id', $r->id)->firstOrFail();
        ...
        return new BankTransaction(new BankTransactionId(new Uuid($r->id)), new AdministrationBankAccountId(new Uuid($r->administration_bank_account_id)), new AdministrationId(new Uuid($r->administration_id)), new TransactionDate(new DateTimeImmutable($r->transaction_date)), new Money($r->amount, new Currency($r->currency)), new BankTransactionReference($r->reference), new TransactionDescription($r->description), new Payment(new PaymentId(new Uuid($p->id)), PaymentType::from($p->type), new RelationId(new Uuid($p->relation_id)), new Money($p->amount, new Currency($p->currency)), $alloc), BankTransactionStatus::from($r->status), new UserId(new Uuid($r->created_by)), new DateTimeImmutable($r->created_at), $r->finalized_by === null ? null : new UserId(new Uuid($r->finalized_by)), $r->finalized_at === null ? null : new DateTimeImmutable($r->finalized_at));
    }
}
