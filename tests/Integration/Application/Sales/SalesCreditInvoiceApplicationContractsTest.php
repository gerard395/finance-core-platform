...
final class SalesCreditInvoiceApplicationContractsTest extends TestCase
{
    ...
    public function test_create_derives_full_credit_truth_and_roundtrips_snapshots_and_reads(): void
    {
        ...
        self::assertSame('DE123456789', $credit->customerFiscalSnapshot()?->vatIdentificationNumber()?->toString());
        self::assertSame('DE', $credit->customerFiscalSnapshot()?->fiscalJurisdiction()?->value());
        self::assertSame('NL123456789B01', $credit->supplierFiscalSnapshot()?->vatIdentificationNumber()?->toString());
        self::assertSame('NL', $credit->supplierFiscalSnapshot()?->fiscalJurisdiction()?->value());
        self::assertSame('2026-08-19', $credit->originalSupplyDate()?->value()->format('Y-m-d'));
    ...
    private function seedTenant(string $admin, int $tenant): void
    {
        ...
        foreach ([1 => ['VAT21', '21'], 2 => ['VAT9', '9']] as $n => [$code, $rate]) {
            DB::table('tax_codes')->insert(['id' => $this->id($tenant, 70, $n), 'administration_id' => $admin, 'code' => $code, 'name' => $code, 'rate' => $rate, 'direction' => 'output', 'status' => 'active', 'treatment' => 'domestic_standard', 'vat_return_classification' => 'domestic_standard', 'icp_classification' => 'none', 'created_at' => $now, 'updated_at' => $now]);
        }
