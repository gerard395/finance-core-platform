...
final class PostSalesCreditInvoiceWithTaxTest extends TestCase
{
    ...
    {
        return (new PostSalesCreditInvoiceWithTax(new TaxPostingReversalPolicy, new TaxPostingIdentityPolicy, new PostingEngine(new PostingValidation, fn () => new JournalEntryId($this->uuid('a')))))->execute(
    ...
    private function input(SalesCreditInvoiceLine $line, TaxPosting $original, bool $taxLine = true, ?JournalEntryLineId $revenue = null, ?TaxPostingId $reversalId = null): SalesCreditFiscalLineInput
    {
        return new SalesCreditFiscalLineInput($line->id(), $original, new LedgerAccountId($this->uuid('c')), new LedgerAccountId($this->uuid('c')), $revenue ?? $this->lineId(), $taxLine ? $this->lineId() : null, $reversalId ?? new TaxPostingId($this->uuid('6')));
    }
